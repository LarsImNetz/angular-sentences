...
(function (gobal, angular) {
        ...
        .constant("modelFormatConfig", {
            "currencyNoSymbol": {
                "formatter": function (args) {
                    ...
                    var removeCurrencySigns = val.replace(/[^0-9.,]/g, "");
                    val = removeCurrencySigns;
                ...
                "parser": function (args) {
                    ...
                    var removeDotRegExp = viewValue.replace(/\./g, "");
                    var replaceCommaWithDot = removeDotRegExp.replace(",", ".");
                    var num = replaceCommaWithDot.replace(/[^0-9.]/g, "");
                    var result = parseFloat(num, 10); //HINT: parseFloat kommt nicht mit deutscher Formatierung zurecht (Bsp: 1.000.000,00 -> 1)
                    return isNaN(result)
                        ? undefined
    ...
    var currencyKeyBoard = function (event, viewValue) {
        ...
        return (viewValue.toString()
            .indexOf("$") === -1 && which === 52 && event.shiftKey);
    };

    var floatKeyBoard = function (event, viewValue) {
        var which = event.which;
        return [188].indexOf(which) != -1 || (which === 190 || which === 110) && viewValue.toString()
                .indexOf(".") === -1;
    };
